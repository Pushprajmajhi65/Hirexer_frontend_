...
import { useEditMeeting } from "@/services/meeting";  // Updated to use useEditMeeting
...
const EditMeeting = ({ onClose, meetingData }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      members: meetingData?.members || [{ email: "" }],
    },
  });

  const mutation = useEditMeeting(); // Using the edit meeting hook
  ...
  const onSubmit = (data) => {
    ...
    mutation.mutate(
      {
        meetingId: meetingData.id, // Passing the meeting ID for updating
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="flex justify-end gap-3 pt-6">
          ...
          <Button type="submit" disabled={mutation.isPending}>
            {mutation.isPending ? "Updating..." : "Update Meeting"}
          </Button>
        </div>
      </form>
    </div>
